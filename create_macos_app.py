...
def _launcher_sh(port: int, py_script: Path, pid_file: Path, log_file: Path) -> str:
    """Shell script that properly daemonises the Flask server."""
    python_bin = _find_python()
    return f'''\
#!/bin/sh
# auphonic-oss server launcher — generated by create_macos_app.py
export PATH=/opt/homebrew/bin:/usr/local/bin:$PATH
mkdir -p "{pid_file.parent}"
nohup "{python_bin}" "{py_script}" serve --port {port} \\
    </dev/null >>"{log_file}" 2>&1 &
echo $! > "{pid_file}"
'''


def _applescript(launcher_sh: Path, port: int, log_file: Path) -> str:
    url = f"http://localhost:{port}/"
    return f'''\
on run
    set theURL to "{url}"
    set theLauncher to "{launcher_sh}"
...
def main():
    ...
    # castpolish.py is bundled inside the app to avoid macOS Documents-folder
    # privacy restrictions that block apps from accessing ~/Documents at runtime.
    bundled_server = resources_dir / "castpolish.py"
    script = _applescript(launcher_sh, SERVER_PORT, LOG_FILE)
    _compile_app(app_path, script)
    # Inject launcher.sh and a fresh copy of castpolish.py into the bundle
    launcher_sh.write_text(_launcher_sh(SERVER_PORT, bundled_server, PID_FILE, LOG_FILE))
